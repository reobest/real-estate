...
const FiltersFull = () => {
  ...
  const isFiltersFullOpen = useAppSelector(
    (state: any) => state.global.isFiltersFullOpen
  );
  ...
  const updateURL = debounce((newFilters: FiltersState) => {
    ...
    Object.entries(cleanFilters).forEach(([key, value]: any) => {
...
const handleSubmit = async () => {
  ...
  // If location is provided, perform geocoding
  if (localFilters.location) {
    try {
      ...
      if (data.features && data.features.length > 0) {
        ...
        updatedFilters.coordinates = [lng, lat];
      }
    } catch (err) {
      ...
      <div className="flex flex-col space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between py-4 gap-4">
            {Object.entries(PropertyTypeIcons).map(([type, Icon]) => (
              <div
                key={type}
                className={cn(
                  "group flex flex-col items-center justify-center p-4 border rounded-xl cursor-pointer transition-colors",
              ...
              >
                ...
                <span className="transition-colors">{type}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setLocalFilters((prev: any) => ({ ...prev, beds: value }))
              }
            >
          ...
          <div className="flex-1">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setLocalFilters((prev: any) => ({ ...prev, baths: value }))
              }
            >
              <SelectTrigger className="w-full rounded-xl">
        ...
        <div className="flex gap-4 mt-6">
          <Button
            ...
            className="flex-1 bg-black text-white rounded-xl cursor-pointer"
          >
            APPLY
          </Button>
          <Button
