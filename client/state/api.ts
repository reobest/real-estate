...
export const api = createApi({
  ...
  endpoints: (build) => ({
    ...
    getProperties: build.query<Property[], Partial<FiltersState> & { favoriteIds?: number[] }>({
      ...
      providesTags: (result) => result ? [...result.map(({ id }: { id: string }) => ({ type: "Properties" as const, id })), { type: "Properties", id: "LIST" },]
...
export const { useGetAuthUserQuery, useUpdateTenantSettingsMutation,
  useUpdateManagerSettingsMutation, useGetPropertiesQuery,
  useAddFavoritePropertyMutation, useRemoveFavoritePropertyMutation, 
  useGetTenantQuery, useGetPropertyQuery,useCreateApplicationMutation,useGetCurrentResidencesQuery,  useGetLeasesQuery,
  useGetPaymentsQuery,
